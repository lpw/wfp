...
export const flightPlansPointCountQuery = f => connectionQuery( `SELECT flightplans.id, COUNT(*) as pointCount FROM flightplans, flightplans_routes, routes, routes_points WHERE flightplans.id=flightplans_routes.flightplan AND flightplans_routes.route=routes_points.route AND routes_points.route=routes.id GROUP BY flightplans.id`, f )
